...
export class LoadService {
  ...
  async saveForm(form: FormGroup): Promise<number> {
    ...
    const accessToken = this.userService.getUser().access_token;

    const requestBody = this.createRequestBody(form);
  ...
  private createRequestBody(form: FormGroup<any>) {
    const expirationDate: Date = new Date(form.get('expired')?.value);
    const duration: number = Math.ceil(Math.abs(expirationDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
  ...
  // saveImage(form: any): void {
  //   const fileControl = form.get('file');
  //   if (fileControl && fileControl.files.length > 0) {
  //     const imageData = new FormData();
  //     imageData.append('file', fileControl.files[0]);
  //
